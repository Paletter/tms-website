define(function() {
    ...
    function($scope, BaseHttp, AppConfig, Dialog, $rootScope, $routeParams, PrintService, $timeout, $location) {
        ...
        //物流号格式化
        $scope.delLogisticNoPreCode = function() {
                ...
                if(data.result) {
                    ...
                    BaseHttp.post("/PiecesStockInController/queryStockedPiecesInfo",param).success(function(data2) {
                        if(data2.result) {
                            ...
                            JQueryUtils.focusSelect("logisticNo");
                        }
                    });
        ...
        $scope.stockInValidate = function() {
            ...
            if(!$scope.piecesResultVo.memberCode) {
                Dialog.alert("客户代码不能为空。", {ok:function() {}, time:4});
                return false;
            }
            if(!$scope.piecesResultVo.logisticsNo) {
